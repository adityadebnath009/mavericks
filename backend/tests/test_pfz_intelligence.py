...
def test_pfz_routing():
    """
    Validates Dijkstra-based route calculations and metrics extraction.
    """
    mock_db = MagicMock()
    ...
    with patch("app.api.services.pfz_routing.evaluate_geofence_offline") as mock_geo:
        ...
        assert res is not None
        assert "route_coords" in res
        assert len(res["route_coords"]) >= 2
        assert "straight_coords" in res
        assert "summary" in res
        
        summary = res["summary"]
        assert "distance_km" in summary
        assert summary["distance_km"] > 0.0
        assert "travel_time_hours" in summary
