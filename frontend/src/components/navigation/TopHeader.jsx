...
const MODE_TITLES = {
  routing: {
    tag: 'TACTICAL ROUTING',
    desc: 'A* Safe Pathfinding & Boundary Geofencing'
  },
  fisheries: {
    tag: 'OCEAN ANALYTICS',
    desc: 'Potential Fishing Zones & Thermal Fronts'
  },
  weather: {
    tag: 'METEOROLOGICAL HAZARDS',
    desc: 'SVAS Capsizing BSI & Severe Weather'
  }
};
...
export function TopHeader({
  ...
  return (
    <header className="h-12 min-h-[48px] border-b border-[#20384D] bg-[#0D1B2A] px-4 sm:px-6 flex items-center justify-between text-xs select-none z-20 shrink-0">
      ...
      {/* Left HUD: Back Button + NAVIK Brand + Active Workspace Tag */}
      <div className="flex items-center gap-3">
        {onBackToLanding && (
          <button 
            onClick={onBackToLanding}
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#13263A] hover:bg-[#1b344e] text-[#00D4FF] border border-[#20384D] hover:border-[#00D4FF]/40 transition-colors font-mono text-[10px] font-bold cursor-pointer"
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="font-extrabold text-[#EAF4F8] uppercase tracking-widest text-xs font-mono">
                NAVIK // CONSOLE
              </span>
              <span className="hidden lg:inline text-[9px] font-mono px-1.5 py-0.5 rounded bg-[#00D4FF]/10 text-[#00D4FF] border border-[#00D4FF]/30 font-bold">
                [ {modeInfo.tag} ]
              </span>
            </div>
            <span className="text-[9px] text-[#8FA8B8] hidden sm:block">
              {modeInfo.desc}
            </span>
          </div>
        </div>
      ...
      {/* Center HUD: Coordinates & Spatial Geofence Status */}
        ...
        <div className="bg-[#07111F] px-3 py-1 rounded-lg border border-[#20384D] flex items-center gap-3 text-[#8FA8B8]">
          ...
          <div>
            ...
            <span className="text-[#00D4FF] font-bold">
              {distToBorder != null ? `${Number(distToBorder).toFixed(1)} km` : 'EEZ CLEAR'}
            </span>
          </div>
        </div>

        {/* Global Operational Risk Pill */}
        <RiskBadge level={overallRisk} size="sm" />
      </div>

      {/* Right HUD: Data Telemetry Status, Timestamp & Actions */}
      <div className="flex items-center gap-4 text-[#8FA8B8] font-semibold">
        ...
        <div className="hidden xl:flex items-center gap-3 text-[10px] font-mono">
          <div className="flex items-center gap-1.5 bg-[#13263A] border border-[#20384D] px-2 py-0.5 rounded text-[#18C7A0]">
            <div className="w-1.5 h-1.5 rounded-full bg-[#18C7A0] animate-pulse" />
            <span>INCOIS: LIVE</span>
          </div>
          <div className="flex items-center gap-1.5 bg-[#13263A] border border-[#20384D] px-2 py-0.5 rounded text-[#00D4FF]">
            <Waves className="w-3 h-3 text-[#00D4FF]" />
            <span>WW3: ACTIVE</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 border-l border-[#20384D] pl-3 sm:pl-4">
          {onRefresh && (
            <button 
              onClick={onRefresh}
              disabled={isLoading}
              className="p-1.5 bg-[#13263A] hover:bg-[#1b344e] text-[#EAF4F8] rounded-lg border border-[#20384D] hover:border-[#00D4FF]/40 transition active:scale-95 cursor-pointer"
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2.5 py-1 rounded-lg border font-mono text-[10px] font-bold transition-all cursor-pointer ${
                isChatOpen
                  ...
                  : 'bg-[#00D4FF]/10 hover:bg-[#00D4FF]/20 border-[#00D4FF]/40 text-[#00D4FF]'
              }`}
              title="Open AI Safety Advisor (Grounded RAG)"
            >
